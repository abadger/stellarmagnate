...
class UrwidInterface(Interface):
    def __init__(self, adapter):
        self.adapter = adapter

        self.title_card = TitleCard()
        urwid.connect_signal(self.title_card, 'close_title_card', self.adapter.main)

        self.main_screen = MainScreen()
        placeholder = urwid.Filler(urwid.Text(''))
        self.urwid_loop = urwid.MainLoop(placeholder,
                event_loop=urwid.AsyncioEventLoop(loop=adapter.event_loop))
                #unhandled_input=self.toplevel_input)

    def display_splash(self):
        self.urwid_loop.widget = self.title_card

    def display_main(self):
        self.urwid_loop.widget = self.main_screen
        pass

    def display_market(self):
        commodity_list = []
        for commodity in self.adapter.planets[self.adapter.player.location].commodities:
            button = urwid.Button(commodity)
            urwid.connect_signal(button, 'click', self.commodity_chosen)
            commodity_list.append(button)

        market_win = urwid.ListBox(commodity_list)
        self.urwid_loop.widget = market_win

    def display_navigation(self):
        planet_list = []
        for planet in (p for p in self.adapter.planets if p != self.adapter.player.location):
            button = urwid.Button(planet)
            urwid.connect_signal(button, 'click', self.move_to)
            planet_list.append(button)

        movement_win = urwid.ListBox(planet_list)
        self.urwid_loop.widget = movement_win
